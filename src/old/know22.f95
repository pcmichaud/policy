...
double precision beta_0, gamma, umin							! preferences
...
double precision, allocatable :: beta(:)						! heterogeneity beta
	...
	subroutine get_ev(s , e, y, o, fn0, an, n, vnext, ev)
		...
		select case(env)
			...
			case (1)
				...
				R = factor(fn, gridd(1), 0.0d0, 0.0d0, e)
				ar = R*an
			...
			case (2)			
				do k = 1, nr, 1
					R = factor(fn, gridd(n), pter(k), 0.0d0,e)
					ar = R*an
					a0 = floor((dsqrt(ar) - dsqrt(amin))/gapa)+1
					if (a0.lt.1) then
						ar = amin
			...
			case (3)
				R = factor(fn, gridd(n), 0.0d0,0.0d0,e)
				ar = R*an
				a0 = floor((dsqrt(ar) - dsqrt(amin))/gapa)+1
				if (a0.lt.1) then
					ar = amin
			...
			case (4)
				do k = 1, nr, 1
					do j = 1, ny, 1
						R = factor(fn, gridd(n), pter(k), ptey(e,j),e)
						ar = R*an
						a0 = floor((dsqrt(ar) - dsqrt(amin))/gapa)+1
						if (a0.lt.1) then
							ar = amin
				...
				do s = 1, T-1, 1
					...
					if (dead(i,s).eq.1) then
					   ...
					   ! intrapolate value function (to get utility)
						...
						R = factor(fsim(i,s+1), dsim(i,s+1), pter(rsim(i,s+1)), ptey(esim(i,s),ysim(i,s+1)),esim(i,s))
