...
contains
	...
	!								write(*,20) inc,med,eqcmin
			...
			write(fileid,'(I4)') T
			fileid = adjustl(fileid)
			filename = '/brinker-5/a/michaud/fin/data/vnext_' // trim(fileid) // '.csv'
		...
		do state = 1, nstate, 1
			...
			select case (solution)
				...
				case (1)
					...
					value(state) = u + (1.0d0-mort(s,e))*beta*ev + mort(s,e)*bequest(an) 
					...
					do while (dif_c.gt.eps_c .or. dif_i.gt.eps_i)
						...
						do c = 1, nc, 1
							...
							do i = 1, ni, 1
								...
								fn = z + in
								call get_ev(s , e, y, o,  fn, an, 1, vnext, ev)
								guess = u + (1.0d0-mort(s,e))*beta*ev + mort(s,e)*bequest(an)
			...
			do i = 1, nsim, 1
				...
				do s = 1, T-1, 1
					...
					if (dead(i,s).eq.1) then
						...
						isim(i,s) = is
						if (is.gt.0.0d0 .and. esim(i,s).eq.2) then
