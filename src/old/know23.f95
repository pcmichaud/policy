...
double precision eta											! heterogeneity beta
...
double precision, allocatable :: csim(:,:), isim(:,:), fsim(:,:), asim(:,:), &
	vsim(:,:), dsim(:,:)
integer, allocatable :: ysim(:,:), osim(:,:), dead(:,:)
...
contains
						...
						eqcmin = cmin*eq(T,e)/((2.7d0)**0.7)
							do f = 1, nf, 1
								do a = 1, na, 1
									...
									call grid(y,o,q)
									vopt(T,q,e,f,a) = eq(T,e)*utility(x/eq(T,e),e)
									copt(T,q,e,f,a) = x
									dopt(T,q,e,f,a) = umin
		...
		do state = 1, nstate, 1
			...
			select case (solution)
				...
				case (2)
					...
					optv = umin
					optd = 0.0d0
					call optimize(xn, z, s, e, y, o, 1, vnext, optc, opti, optd, optv)
					value(state) = optv
					dvalue(state) = 0.0d0
					cvalue(state) = optc
					ivalue(state) = opti
				...
				case (3)
					...
					if (inotech.eq.0) then					
						call optimize(xn, z, s, e, y, o, 2, vnext, optc_s, opti_s, optd_s, optv_s)
					end if			
	...
	subroutine optimize(xn, z, s, e, y, o, n, vnext, optc, opti, optd, optv)
		...
		if (n.eq.1) then
			...
			par(1) = xn
			par(2) = 0.0d0
			fn(1)  = funcvalue(par,xn,z,s,e,y,o,n,vnext)
			! evaluate function where c = eqcmin
			par(1) = eqcmin
			par(2) = 0.0d0
			fn(2) = funcvalue(par,xn,z,s,e,y,o,n,vnext)
		else
			npar = 3
			initpar(1) = 0.3d0*xn + 0.7d0*eqcmin
			initpar(2) = 5.0d0
		...
		select case (solution(1))
			...
			if (optc .gt. xn) then
			!	write(*,*) 'budget constraint not respected'
			end if	
	...
	subroutine get_ev(s , e, y, o, fn0, an, nn, vnext, ev)
		...
		select case(env)
			...
			case (2)			
				do k = 1, nr, 1
					R = factor(fn, nn,xtabr(k), 0.0d0,e)
					ar = R*an
					a0 = floor((rr(ar,1) - rr(amin,1))/gapa)+1
					if (a0.lt.1) then
						ar = amin
			...
			case (4)
				do k = 1, nr, 1
					do j = 1, ny, 1
						R = factor(fn, nn, xtabr(k), ptey(e,j),e)
						ar = R*an
						a0 = floor((rr(ar,1) - rr(amin,1))/gapa)+1
						if (a0.lt.1) then
							ar = amin
				...
				do s = 1, T-1, 1
					...
					if (dead(i,s).eq.1) then
					   ...
					   ! intrapolate value function (to get utility)
						...
						R = factor(fsim(i,s+1), dsim(i,s+1), quann(rdraws(i,s)), ptey(esim(i,s),ysim(i,s+1)),esim(i,s))
						rsim(i,s+1) = R
	...
	double precision function utility(x, e)
		double precision x
		integer e
		if (x.gt. phi*cmin) then
				utility = ((x-phi*cmin)**(1.0d0-gamma(e)))/(1.0d0-gamma(e)) 
			else
				utility = umin
		end if
	end function
