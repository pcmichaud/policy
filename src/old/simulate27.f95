...
module simulate
	...
	contains
		...
		subroutine get_modulename
			...
			write(*,*) '  final revision (2015): simulation of scenarios										   '
			write(*,*) '***********************************************************'
		...
		subroutine runlog(pop,nobs)
			...
			open(1,file=trim(path)//'data/tab-fig/appendix_'//trim(scenario)//'.tex')
		...
		subroutine tab3learn(irun)
			...
			open(1,file=trim(path)//'data/tab-fig/table-3-learn.txt')
			write(*,*) ''			
			write(*,*) '- lowering retirement benefits'
			do i = 1, 5, 1
				write(*,*) i, table(1,i,:) 
		...
		subroutine fig7(irun)
			...
			! report results
			write(*,*) ''			
			write(*,*) '*** figure 7 results'
			open(1,file=trim(path)//'data/tab-fig/fig-7.txt')
			do i = 1, 3, 1
				write(*,*) i, table(i,:)
				write(1,*) i, table(i,:) 
			end do
		...
		subroutine fig8(irun)
			...
			open(1,file=trim(path)//'data/tab-fig/fig-8.txt')
			do s = 1, T, 1
				if (age(s).le.90.0d0) then
					write(*,*) age(s), table(1,s,:), table(2,s,:), table(3,s,:), table(4,s,:)
					write(1,*) age(s), table(1,s,:), table(2,s,:), table(3,s,:), table(4,s,:)
		...
		subroutine fig9(irun)
			...
			open(1,file=trim(path)//'data/tab-fig/fig-9.txt')
			do s = 1, T, 1
				if (age(s).le.90.0d0) then
					write(*,*) age(s), table(1,s,:), table(2,s,:), table(3,s,:), table(4,s,:)
					write(1,*) age(s), table(1,s,:), table(2,s,:), table(3,s,:), table(4,s,:), &
		...
		subroutine fig10(irun)
			...
			write(*,*) ''
			write(*,*) '*** figure 10 results'
			open(1,file=trim(path)//'data/tab-fig/fig-10.txt')
			do s = 1, T, 1
				if (age(s).le.90.0d0) then
		...
		subroutine adhoc(irun)
			...
			! report results for Figure 10
			write(*,*) ''
			write(*,*) '*** figure adhoc results'
			open(1,file=trim(path)//'data/tab-fig/fig-genes.txt')
			do s = 1, T, 1
				if (age(s).le.90.0d0) then
					write(*,*) age(s), table(1,s,:), table(2,s,:), table(3,s,:), table(4,s,:)
					write(1,*) age(s), table(1,s,:), table(2,s,:), table(3,s,:), table(4,s,:)
...
program main
	...
	call fig6(0)	
	! old data for figure 7 (drra)
	call fig7(0)	
	! data for figure 7 (epstein-zin preferences)
    call fig8(0)
