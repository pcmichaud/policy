...
double precision rmin, rmax, rbar, sigr, rhor		! interest rate parameters
double precision k0, k1, pi0,pi1					! cost of technology
...
double precision, allocatable :: dopt(:,:,:,:,:)	! utility diff technology
...
contains
	...
	subroutine initpar
		...
		open(unit=1, file='/brinker/a/michaud/fin/data/structural-parameters.asc')
		...
		open(unit=1, file='/brinker/a/michaud/fin/data/other-parameters.asc')
		read(1,*) ! reading header
		read(1,*) amin
		read(1,*) amax
		read(1,*) fmin
		...
		open(unit=1, file='/brinker/a/michaud/fin/data/dimensions-parameters.asc')
		read(1,*) ! reading header
		read(1,*) T
		read(1,*) ny
		read(1,*) no
		...
		retage(3) = retage(1)
		! set path 
		path = '/brinker/a/michaud/fin/data/'
		! minimum utility
		umin = -10.0d6
	...
	subroutine statespace
		...
		gapa = (amax - amin)/dble(na-1)
		gapf = (fmax - fmin)/dble(nf-1)
		!write(*,*) gapa, gapf
		do i=1, na, 1
			grida(i) = amin + dble(i-1)*gapa
		...
		allocate(mort(T,ne))
		open(unit=2,file='/brinker/a/michaud/fin/data/mortality1940.csv') 
		do i =  1 ,T, 1 
			mort(i,:) = 0.0d0
			read(2,*) mort(i,:)	
			!write(*,*) i,mort(i,:)
		...
		allocate(eq(T,ne))
		! loading coefficients
		open(unit=3,file='/brinker/a/michaud/fin/data/equivalence.csv') 
		!read(3,*) header
		do i =  1 ,T 
			eq(i,:) = 0.0d0
			read(3,*) eq(i,:)	
		...
		ns = nq*ne*nf*na
		maxsize = ceiling(dble(ns+1)/dble(numworkers))
	...
	subroutine income_shocks
		...
		! read parameters		
		open(unit=1, file='/brinker/a/michaud/fin/data/incomeworkpar.csv')
		do i = 1, 6, 1
			read(1,*) bw(i,:)
		end do
		close(unit=1)
		...
		do e = 1, 3, 1
			...
			rho = bw(4,e)
			sige = dsqrt(bw(5,e))
			sig = 0.5d0*dsqrt(bw(6,e)) + sige/dsqrt(1.0d0-rho**2) 
			incsig(e) = sig**2
			! grid  points
			ptey(e,1) = -m*sig
			ptey(e,ny) = m*sig
		...
		! read parameters		
		open(unit=1, file='/brinker/a/michaud/fin/data/ret-income.csv')
		read(1,*) ; ! reading header
		do i = 1, 36, 1
			read(1,*) br(i,:)
			write(*,*) br(i,:)
	...
	subroutine oop_shocks
		...
		! read parameters		
		open(unit=2, file='/brinker/a/michaud/fin/data/ooppar.csv')
		do i = 1, 6, 1
			read(2,*) bo(i,:)
		end do
		close(unit=2)
		...
		do e = 1, ne, 1
			...
			rho = bo(4,e)
			sige = dsqrt(bo(5,e))
			sig = 0.5d0*dsqrt(bo(6,e)) + sige/dsqrt(1.0d0-rho**2) 
			! grid  points
			pteo(e,1) = -m*sig
			pteo(e,no) = m*sig
			gap = (2.0d0*m*sig)/no1
	...
	subroutine rate_shocks
		...
		!write(*,*) 'nf = ', factor(gridf(10), 2, 0.0d0)
			!do i = 1, nf, 1
				!write(*,*) gridf(i),factor(gridf(i), 1, 0.0d0), factor(gridf(i), 2, pter(1)), factor(gridf(i), 2, pter(nr))
				!call flush()
			!end do
	...
	!								write(*,20) inc,med,eqcmin
			...
			open(unit=1,file='/brinker/a/michaud/fin/data/vnext.csv')
			do y = 1, ny, 1
				do o = 1, no, 1
					call grid(y,o,q)
					do e = 1, ne, 1
	...
	subroutine recursion
		integer s, y, o, e, f, a, j, k, q, i
		integer nstate, state, sent, received, jobsize, nstates, index(4)
		integer sender, tag
		double precision result(maxsize,maxsize), value(maxsize)
		...
		if (ismaster) then						! Master's code
			...
			do s=T-1, 1, -1	
				...
				sent = 0
				jobsize = ceiling(dble(ns+1)/dble(numworkers))
				index(1) = 1
				! send jobs
				do j = 1, numworkers, 1					
					! check that last index not ns
					index(2) = index(1) + jobsize - 1
					if (index(2).gt.ns) then
						index(2) = ns
				...
				do y = 1, ny, 1
					do o = 1, no, 1
						...
						do e = 1, ne, 1
								do f = 1, nf, 1
									do a = 1, na, 1				
										write(1,*) vopt(s,q,e,f,a),copt(s,q,e,f,a),iopt(s,q,e,f,a)
									end do
								end do
						end do
					end do
			...
			!call flush()
			! send message to workers that we are done (tag 2)
			do j = 1, numworkers, 1
				call mpi_send(index,4,MPI_INTEGER,j,2,MPI_COMM_WORLD,ier)
			end do
			...
			do while (.not. finished)
				...
				!write(*,*) 'worker ',rank,' got a message...'
				...
				if (tag.eq.2) then
					...
					! perform job
					call get_opt(index(4) ,index(1), index(2), index(3), value, dvalue, cvalue, ivalue)
	...
	subroutine get_opt(s, sbegin, send, nstate, value, dvalue, cvalue, ivalue)
		! argument list
		...
		integer y, o, e, f, a, i, c, q, n, oi
		integer solution, nc, ni
		double precision inc, med,  eqcmin, z, bound_c(2), bound_i(2)
		double precision x, cost, guess, xn, oldc, oldi
		double precision cn, an, fn, in, u, ev, gapi, gapc,dif_c,dif_i,gapi_max
		double precision maxv, maxvd(2), enough, maxc(2), maxi(2), maxvf
		...
		do state = 1, nstate, 1
			...
			! provide consumption floor if necessary
			if (x.lt.eqcmin) then
				x = eqcmin
				tr = .true.
			else
			...
			select case (solution)
				...
				case (2)
					maxvd(1) = umin
					maxvd(2) = umin
					nc = int(dble(na)/1.5d0)
					ni = int(dble(nf)/1.5d0)
					xn = x
					bound_c(1) = eqcmin
					bound_c(2) = xn
					...
					oldc = 0.0d0
					oldi = 0.0d0
					do while (dif_c.gt.eps_c .or. dif_i.gt.eps_i)
						...
						! update consumption bounds
						bound_c(1) = max(maxc(1) - gapc,eqcmin)
						bound_c(2) = min(maxc(1) + gapc,xn)
						
						
						bound_i(1) = max(maxi(1) - gapi_max,0.0d0)
						bound_i(2) = min(maxi(1) + gapi_max,fmax - z)
						
						! difference
						dif_c = dabs(maxc(1) - oldc) 
						dif_i = dabs(maxi(1) - oldi)
						!write(*,*) 'doing again ', eqcmin,xn,state,maxc(1),dif_c,bound_c(1),bound_c(2)
						!write(*,*) 'doing again ', z,state,maxi(1),dif_i,bound_i(1),bound_i(2)

						! keep to check if better
						oldc = maxc(1) 
						oldi = maxi(1)
				...
				case (3)
					...
					do n = 1, nd, 1	
						...
						! start with coarse grid
						nc = int(dble(na)/2.0d0)
						ni = int(dble(nf)/2.0d0)
						...
						do while (dif_c.gt.eps_c .or. dif_i.gt.eps_i)
							...
							! update consumption bounds
							bound_c(1) = max(maxc(n) - gapc,eqcmin)
							bound_c(2) = min(maxc(n) + gapc,xn)
							
							
							bound_i(1) = max(maxi(n) - gapi_max,0.0d0)
							bound_i(2) = min(maxi(n) + gapi_max,fmax - z)
							
							! difference
							dif_c = dabs(maxc(n) - oldc) 
							dif_i = dabs(maxi(n) - oldi)
							!write(*,*) 'doing again ', xn,state,n,maxc(n),dif_c,bound_c(1),bound_c(2)
							!write(*,*) 'doing again ', z,state,n,maxi(n),dif_i,bound_i(1),bound_i(2)
						...
						! ****** end of need to iterate over grid ******
						

					end do
					
					! assign maximum over technology choice
					dvalue(state) = maxvd(2) - maxvd(1)
					if (dvalue(state).gt.0.0d0) then
						value(state) = maxvd(2)
						cvalue(state) = maxc(2)
						ivalue(state) = maxi(2)
					else
					...
					!if (maxi.gt.0.0d0) then
						!write(*,*) s,q,e,d,f,a,solution, maxi
					!end if	
			end select												
	...
	subroutine get_ev(s , e, y, o, fn, an, n, vnext, ev)
		...
		select case(env)
			...
			case (1)
				...
				au = (ar - grida(a0))/(grida(a1)-grida(a0))
	...
	subroutine simulate
		integer q, a0, a1, f0, f1, i, s, j
		double precision eqcmin, au, fu, dtag(nf,na), ctag(nf,na), itag(nf,na)
		character (len=80) :: header
		double precision earnsim, ds, cs, is, inc, z, x, R, draw, vproby(ny), vprobo(no), age 	
		...
		if (ismaster) then
			...
			open(unit=3,file='/brinker/a/michaud/fin/data/initsample.csv') 
			read(3,*) header
			do i = 1, nsim, 1
				...
				if (esim(i,1).eq.1) then
					fsim(i,1) = 30.0d0 + 26.8d0*quann(draw)
				else if (esim(i,1).eq.2) then
					fsim(i,1) = 48.15d0 + 26.8d0*quann(draw)
				else
					fsim(i,1) = 66.3d0 + 24.8d0*quann(draw)
				end if
				...
				do s = 1, T-1, 1
					if (dead(i,s).eq.1) then
						...
						incsim(i,s) = inc
						!write(*,*) 'income is ',s,incsim(i,s-1)
						!write(*,*) 'factor is ',s,R
						! net financial knowledge
						...
						asim(i,s+1) = x - csim(i,s) - fcost(isim(i,s)) - oopsim(i,s)
						if (dsim(i,s+1).eq.2 ) then
							asim(i,s+1) = asim(i,s+1) - k0*exp(k1*z)
						end if 
			...
			if (ismaster) then
				write(*,*) '- cohort simulated, savinv data ...'
			end if				
			
			open (2,file='/brinker/a/michaud/fin/data/simknow.dat',status='old')
			do i=1,nsim,1
				do s = 1, T-1, 1
					age = real(24+s)
					if (dead(i,s).eq.1) then
						write(2,*) i,age,csim(i,s),asim(i,s),incsim(i,s),oopsim(i,s),rsim(i,s),isim(i,s),fsim(i,s),esim(i,s),dsim(i,s)
					end if
				end do
			end do	
			close(2)
	...
	double precision function factor(fpt, tech, shock)
		double precision fpt, rf, shock
		integer tech
		if (tech.eq.2) then 
			!write(*,*) 'variance is ',(dexp(rho0+rho1*fpt)/100.0d0)
			rf = rmax*(fpt/fmax) + sigr*shock
			!rf = -rmax
			!rf = rmin + (rmax-rmin)*(fpt/100.0d0) + shock
			factor = 1.0d0 + rf + rbar - inflation
		else 
			factor = 1.0d0 + rbar - inflation
		end if
	...
	double precision function earn(s, e, py)
		...
		double precision py, age, agesq
			age = dble(25+s)
			agesq = 0.01d0*(age**2)
			earn = dexp(br(3,e) + br(1,e)*age + br(2,e)*agesq -0.5d0*incsig(e) +  py)
	end function
	! compute retirement income
	double precision function retinc(s, e, py)
		integer s, e, age
		double precision work, rep
			age = 25+s
			work = earn(25+retage(e)-1,e,py)
			rep = br(age-64,e)
			retinc = work*rep
	end function
