...
integer idprog, ageprog
double precision pi0prog, cprog_s
...
contains
	...
	subroutine initpar
		...
		if (icagetti.eq.1) then
		    open(1,file='/share/michaud_pi/fin/params/cag2003.csv')
            do e = 1, ne, 1
		...
		if (igparker.eq.1) then
		    open(1,file='/share/michaud_pi/fin/params/gp2002.csv')
            do e = 1, ne, 1
    		    read(1,*) beta(e),gamma(e)        
            end do	
            close(1)	
   ...
   !     end if    
	...
	subroutine saverules
		...
		if (ismaster) then		
			...
			open(1, file='/share/michaud_pi/fin/data/tab-fig/invest_'//trim(scenario)//'.txt')				
			s = 20
			y = nint(dble(ny/2))
			o = int(dble(no)/2.0d0)+1
			f = nint(dble(nf/2))
					...
					if (dead(i,s).eq.1) then
					   ...
					   ! intrapolate value function (to get utility)
                        ...
                        if (idprog.eq.1 .and. s.eq.ageprog) then
                            vtag =  vprog(s,q,esim(i,s),:,:)
                            call blend_102(fu, au, vtag(f0,a0),vtag(f0,a1),vtag(f1,a0), &
                                vtag(f1,a1),vs)
                            if (vs .gt. vsim(i,s)) then
                                psim(i,s) = 1
                                vsim(i,s) = vs
                                ! compute optimal technology
                                call grid(ysim(i,s),osim(i,s),q)
