...
integer idifmx, idifeq, idifss, inofin, inotech, icagetti, igparker, izin
integer idprog, ageprog, idelta
double precision pi0prog, costprog
...
!* array of results for program
double precision, allocatable :: vprog(:,:,:,:)	! value function
double precision, allocatable :: dprog(:,:,:,:)	! share invested in technology
double precision, allocatable :: fprog(:,:,:,:)	! financial knowledge stock
double precision, allocatable :: aprog(:,:,:,:)	! assets
double precision, allocatable :: cprog(:,:,:,:)	! consumption
double precision, allocatable :: iprog(:,:,:,:)	! investment
...
contains
	...
	subroutine initialize
		integer i, e, s
		if (ismaster) write(*,*) '+ running scenario : ', scenario
	...
	subroutine statespace
		...
		! allocate space for solution of program
		allocate(vprog(nq,ne,nf,na))			! value function
		allocate(dprog(nq,ne,nf,na))			! utility diff technology
		allocate(fprog(nq,ne,nf,na))			! financial knowledge stock
		allocate(aprog(nq,ne,nf,na))			! assets
		allocate(cprog(nq,ne,nf,na))			! consumption
		allocate(iprog(nq,ne,nf,na))			! investment
	...
	subroutine recursion
		...
		! get the decision rule for the program
		if (idprog.eq.1) then
            s = ageprog
            ! start year
            if (ismaster) then						! Master's code
                ...
                call flush()
                if (ismaster .and. (s.eq.70 .or. s.eq.50 .or. s.eq.25 .or. s.eq.1)) then
                    write(*,*) 'solving for program at age', 25+ageprog
                    call flush()
                end if
                ...
                do while (received .lt. sent)
                    ...
                    do k = index(1), index(2), 1
                        ...
                        a = lookup(k,5)
                        call grid(y,o,q)
                        vprog(q,e,f,a) = result(i,1)
                        dprog(q,e,f,a) = result(i,2) 
                        cprog(q,e,f,a) = result(i,3)
                        iprog(q,e,f,a) = result(i,4)
                        i = i + 1						
                    end do
                    if ((i-1).ne.index(3)) then
                        write(*,*) 'received do not match by ', sender, i,index(1), index(2), index(3)
                ...
                call get_opt(index(4) ,index(1), index(2), index(3), value, &
                    dvalue, cvalue, ivalue, nextvalue, 1)
	...
	subroutine get_opt(s, sbegin, send, nstate, value, dvalue, cvalue, ivalue, nextvalue, ipg)
		...
		do state = 1, nstate, 1
			...
			! check if participates in program
			if (ipg .eq. 1) then
			    if ( .not. tr) then
    			    x = x - costprog
			    end if
	...
	subroutine optimize(xn, z, s, e, y, o, n, vnext, optc, opti, optd, optv, ipg)
		...
		if (inofin.eq.1 .or. psi.gt.0.0d0) then
			initpar(2) = 0.0d0
			step(2) = 0.0d0
		end if
   ...
   !     end if    
	...
	subroutine saverules
		...
		if (ismaster) then		
			...
			open(1, file='../data/tab-fig/cons_'//trim(scenario)//'.txt')				
			s = 20
			y = nint(dble(ny/2))
			...
			open(1, file='../data/tab-fig/share_'//trim(scenario)//'.txt')				
			s = 20
			y = nint(dble(ny/2))
			o = int(dble(no)/2.0d0)+1
			f = nint(dble(nf/2))
			...
			open(1, file='../data/tab-fig/invest_'//trim(scenario)//'.txt')				
			s = 20
			y = nint(dble(ny/2))
			o = int(dble(no)/2.0d0)+1
			f = nint(dble(nf/2))
	...
	subroutine simulate
		integer q, a0, a1, f0, f1, i, s, j, seed(1), k, nobs, elig(nsim,T), part
				...
				do s = 1, T-1, 1
					...
					if (dead(i,s).eq.1) then
					   ...
					   ! intrapolate value function (to get utility)
                        ...
                        ! if program check whether utility higher in program
                        if (idprog.eq.1 .and. s.eq.ageprog) then
                            ! determine eligibility
                            call random_number(draw)
                            if (draw .lt. 0.5d0) then
                                elig(i,s) = 1
                                vtag =  vprog(q,esim(i,s),:,:)
                                call blend_102(fu, au, vtag(f0,a0),vtag(f0,a1),vtag(f1,a0), &
                                    vtag(f1,a1),vs)
                                
                                ! draw for participation
                                call random_number(draw)
                                if ((vs - vsim(i,s) + 0.0001d0*quann(draw)) .gt. 0.0d0) then
                                    psim(i,s) = 1
                                    part = 1
                                    vsim(i,s) = vs
                                    ! compute optimal technology
                                    call grid(ysim(i,s),osim(i,s),q)
                                    dtag =  dprog(q,esim(i,s),:,:)
                                    call blend_102(fu, au, dtag(f0,a0),dtag(f0,a1),dtag(f1,a0), &
                                        dtag(f1,a1),ds)				
                                    dsim(i,s+1) = ds
                                    ! intrapolate optimal consumption
                                    ctag =  cprog(q,esim(i,s),:,:)
                                    call blend_102(fu, au, ctag(f0,a0),ctag(f0,a1),ctag(f1,a0),&
                                        ctag(f1,a1),cs)
                                    csim(i,s) = cs							
                                    ! intrapolate optimal investment
                                    itag =  iprog(q,esim(i,s),:,:)
                                    call blend_102(fu, au, itag(f0,a0),itag(f0,a1),itag(f1,a0),&
                                        itag(f1,a1),is)
                                    isim(i,s) = is                                    
                                end if          
						...
						rsim(i,s+1) = R
						asim(i,s+1) = x - csim(i,s) - fcost(isim(i,s),psim(i,s)) 
				...
				do s = 1, T-1, 1
					...
					if (dead(i,s).eq.1) then
						write(8,*) i,age,vsim(i,s),csim(i,s),asim(i,s),incsim(i,s), &
							oopsim(i,s),rsim(i,s),isim(i,s),fsim(i,s),esim(i,s),dsim(i,s),psim(i,s), elig(i,s)
					end if
				end do
			end do	
			close(8)
	...
	! investment cost
	double precision function fcost(i, ipg)
		double precision i
		integer ipg
		if (i.le.0.0d0) then
			fcost = 0.0d0
